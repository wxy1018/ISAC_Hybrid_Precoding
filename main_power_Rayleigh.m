...
%     end
% end
h_user = (randn(N_t, K) + 1j*randn(N_t, K))/sqrt(2);
...
for n_power = 1:length(P_dB_set)
    P_dB = P_dB_set(n_power);
    P_BS = db2pow(P_dB);
%     F_RF = zeros(N_t, N_RF);
%     for m = 1:N_RF
%         F_RF((M*m-M+1):(M*m), m) = ones(M,1);
...
%             for nn = 1:2
    ...
    Phi = F_RF'*A_dot'*inv(R_N)*A_dot*F_RF;
    CRB(n_power) = real(1/trace(F_BB'*Phi*F_BB) / xi0^2 * sigma2_radar);
end
plot(P_dB_set, CRB, '-bo', 'LineWidth', 1.2);
save('results_power_Nt64_Nrf8_SDPv2.mat', 'P_dB_set', 'CRB', 'gamma_th_dB', 'N_t', 'N_RF', 'opt_FBB')
